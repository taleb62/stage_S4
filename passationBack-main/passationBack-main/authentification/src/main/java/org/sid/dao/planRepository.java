...
//    @Query(value="select p.id,p.origine,p.destinataire,p.objet_depense,p.date_previ_lancement,p.date_previ_attribution,p.inpu_budgetaire,p.mnt_estimatif,m.mode_passation from plan_anuell_achat p INNER JOIN mod_passation m ON p.id=m.id where p.id=?1",nativeQuery = true)
...
//    @Query(value="select new org.sid.entites.DTO.PaaProcedure(p.id,p.origine,p.destinataire,p.objetDepense,p.datePreviLancement," +
//            "p.datePreviAttribution,p.inpuBudgetaire,p.mntEstimatif,f.name,f.fileNameOnDisc,f.fkIdTbl,m.modePassation) " +
//            "from plan_anuell_achat p inner join mod_passation m ON p.id=m.id inner join FileDB f on p.id=f.idElm and f.fkIdTbl=1")
